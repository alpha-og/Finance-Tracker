...
const getTransactions = async (
  ...
  log_print("GETS");
  try {
    const user_check = await User.findById(user_id);
    if (!user_check) return res.status(404).send({ message: "User not found" });
    const transactions = await transactionSchema.find({ user: user_id });
